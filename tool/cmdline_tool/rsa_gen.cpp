...
int testReadKey(RSA_HANDLE _h_rsa,const char *pub_file, const char * priv_file) ;


#define EXIT_RSA(retVal) return (retVal) 

ND_CMDLINE_FUNC_INSTANCE(gen_rsa_key_cpp) 
{
	ND_CMDLINE_CHECK_SHOW_HELP(argc, argv,"rsakeycpp -p public_key.c -s private_key.cpp -v version -bits rsa-key-bit ") ;
	...
	if (!_pri_file || !_pub_file){		
		fprintf(stderr, "usage :rsakeycpp -p public_key.c -s private_key.cpp -v version -bits rsa-key-bit\n");
		EXIT_RSA(1) ;
	}

	if (-1==nd_RSAInit(_h_rsa,bits)){

		fprintf(stderr,"Create RS KEY error\n" ) ;
		EXIT_RSA(1) ;
	}
	if(0!=TestRsa(_h_rsa->privateKey)) {
		EXIT_RSA(1) ;
	}
	...
	if (-1==out_pub_file(_pub_file,_h_rsa->publicKey,number_index,md5text)) {
		fprintf(stderr,"out put public key to file %s error \n", _pub_file) ;
		EXIT_RSA(1) ;
	}
	...
	if (-1==out_priv_file(_pri_file,_h_rsa->privateKey,number_index,md5text)) {
		fprintf(stderr,"out put private key to file %s error \n", _pri_file) ;
		EXIT_RSA(1) ;
	}
/*
...
ND_CMDLINE_FUNC_INSTANCE(gen_rsa_key_bin) 
{
	ND_CMDLINE_CHECK_SHOW_HELP(argc, argv,"rsakeybin -p public_key.bin -s private_key.bin -v -bits rsa-key-bit ") ;
	...
	if(-1==testReadKey(_h_rsa,_pub_file,_pri_file) ) {
		fprintf(stderr,"test Read ERROR \n") ;
		EXIT_RSA(1) ;
	}
...
int testReadKey(RSA_HANDLE _h_rsa,const char *pub_file, const char * priv_file)
{
	...
	if(-1==nd_rsa_privkey_input(&priv_key, priv_file) )  {
		printf("read private key bin error \n") ;
		EXIT_RSA(1) ;
	}

	if(-1==nd_rsa_pubkey_input(&pub_key, pub_file) )  {
	...
	void *data = nd_load_file(pub_file, &size) ;
	if (!data ) {
