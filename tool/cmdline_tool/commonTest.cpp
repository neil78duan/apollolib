...
NDIConn* LoginServer( const char * host,int port, const char* user, const char *passwd, const char*sessionFile,NDUINT32 *accID ) 
{
	...
	if (sessionFile && sessionFile[0]) {
		bSuccess =	RloginTrylogin("unknow",pConn->GetHandle(),ACC_APOLLO, user, passwd,sessionFile, accID) ;
	}
	else {
		bSuccess = loginAndCreate("unknow",pConn->GetHandle(),ACC_APOLLO,user, passwd,NULL,accID);
	}
